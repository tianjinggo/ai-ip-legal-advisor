...
description: 面向法务专业人员的 AI 知识产权侵权风险评估与保护咨询助手，支持 AIGC 可版权性、训练数据侵权、AI 产品合规、开源协议、侵权案例与维权策略分析，并可生成 Word 法律分析报告、法律意见书及导出 docx。适用于即时法律咨询、风险评估、正式报告撰写和 Word 文档交付场景。
author: 刘天京
metadata:
  compatibility:
    consultation_mode: 无外部依赖，任意环境可用
    report_mode: Node.js >= 20；首次使用前执行 npm install 安装 jszip 与 xmlbuilder2
---
...
> **⚠️ 环境要求**：报告生成模式需要 Node.js >= 20 运行环境。首次使用前，请在本 skill 目录下执行：
> ```bash
> npm install
> ```
